...
def compLevApprSpe(A, k, q, reo=4):
        ...
        B = np.dot(Q.T, A)
        print "in Spe, computing SVD!"
        [U1, D, V] = np.linalg.svd(B, full_matrices=False)

        U = np.dot(Q.T, U1) 

        lev_row, p_row = compLevExact(U, k, 0)
        #lev, p = compLevApprFro(B, k, r)
        lev_row = p_row*k

        lev_col, p_col = compLevExact(V, k, 1)
        #lev, p = compLevApprFro(B, k, r)
        lev_col = p_col*k

        return lev_row, p_row, lev_col, p_col
