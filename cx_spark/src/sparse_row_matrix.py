...
class SparseRowMatrix(object):
    ...
    def gaussian_projection(self, c):
        ...
        if direct_sum:
            ...
            gp_dict = self.rdd.mapPartitions(lambda records: gaussian_projection_mapper(records,n=n,c=c)).filter(lambda x: x is not None).reduceByKey(add,numPartitions=100).collectAsMap()
